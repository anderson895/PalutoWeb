  ...
  <div class="product-card" @click="$router.push(`/product/${product.id}`)">
    ...
    <div class="product-body">
      ...
      <div class="product-footer">
        ...
        <span class="product-time" v-if="product.prepTime">🕐 {{ product.prepTime }}min</span>
      </div>
    </div>
  </div>
</template>
...
.quick-add { position: absolute; bottom: 10px; right: 10px; width: 36px; height: 36px; border-radius: 50%; background: var(--accent); border: none; color: white; font-size: 20px; font-weight: 300; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; opacity: 0; box-shadow: 0 4px 12px rgba(232,70,42,0.4); }
